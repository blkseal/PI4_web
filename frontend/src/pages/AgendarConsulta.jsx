...
function AgendarConsulta() {
    ...
    const filteredPacientes = pacientes.filter(p => {
        ...
        const nus = p.nus?.toLowerCase() || '';
        return name.includes(pacienteSearch.toLowerCase()) || nus.includes(pacienteSearch.toLowerCase());
    });
    const selectPaciente = (p) => {
        setFormData(prev => ({ ...prev, pacienteId: p.id, pacienteNome: p.nomeCompleto }));
        setPacienteSearch(p.nomeCompleto);
        setShowPacienteDropdown(false);
    };
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.response?.data?.mensagem || 'Erro ao agendar consulta. Verifique os dados.');
        } finally {
            setLoading(false);
        }
    };
                                        ...
                                        {filteredPacientes.length > 0 ? (
                                            filteredPacientes.map(p => (
                                                ...
                                                >
                                                    ...
                                                    <span className="patient-details">NUS: {p.nus || 'N/A'}</span>
                                                </button>
                                            ))
                                        ) : (
                                            <div className="dropdown-item" style={{ cursor: 'default' }}>Nenhum paciente encontrado</div>
                                        )}
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="form-row">
                        ...
                        <div className="form-group">
                            ...
                            <div className="input-with-icon">
                                ...
                                <div className="icon-container" dangerouslySetInnerHTML={{ __html: toothSvg }} style={{ width: '20px', height: '20px' }}>
                                </div>
                            </div>
                        </div>
                        <div className="form-group">
