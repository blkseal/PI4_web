...
function ConsultaDetalheUtente() {
  ...
  useEffect(() => {
    const fetchConsulta = async () => {
      ...
      setError("");
      try {
        // Sem endpoint de detalhe, obter a lista em /home (consultas futuras do utente)
        ...
        // Fallback: procurar na lista de consultas (o backend deve validar permissões)
        if (!found) {
          const respAll = await api.get("/admin/consultas", {
        ...
        <div className="detalhe-card">
          <div className="detalhe-section">
            ...
            <div className="detalhe-item">
              ...
              <span className="value">
                {formatTime(consulta.horaInicio)} -{" "}
                {formatTime(consulta.horaFim) || "—"}
              </span>
            </div>
            <div className="detalhe-item">
              <span className="label">
              ...
              <span className="value">
                {consulta.estado?.toLowerCase() === "pendente"
                  ? "Por Acontecer"
                  : consulta.estado || "Por Acontecer"}
              </span>
            </div>
          </div>
          ...
          <div className="notas-container">
            ...
            <p className="notas-text">
              {consulta.titulo ||
                consulta.notas ||
                "Sem notas adicionais para esta consulta."}
            </p>
          </div>

          {(consulta.valor_estado || consulta.estado || "")
            .toLowerCase()
            .includes("conclu") && (
            <div className="anexos-container">
              ...
              <div className="anexos-list">
                {consulta.anexos && consulta.anexos.length > 0 ? (
                  ...
                  <p className="no-anexos">
                    Sem anexos disponíveis para esta consulta.
                  </p>
                )}
              </div>
            </div>
