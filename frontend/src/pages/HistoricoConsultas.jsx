...
const HistoricoConsultas = () => {
  ...
  const fetchConsultas = async () => {
    try {
      ...
      const formatted = response.data
        .filter((c) => {
          // 1. Filter: Only PAST consultas (date is before today)
          const consultaDate = new Date(c.data);
          consultaDate.setHours(0, 0, 0, 0);
          if (consultaDate >= today) return false;

          // 2. Filter by Estado: Completed, Cancelled, or Annulled only
  ...
  const filteredConsultas = useMemo(() => {
    return consultas.filter((c) => {
      const matchesUtente =
        filterUtente === "" || c.nUtente.toString().includes(filterUtente);
      ...
      return (
        matchesUtente && matchesData && matchesDentista && matchesEspecialidade
      );
    });
  }, [
    consultas,
