...
const ConsultasGestor = () => {
  ...
  const fetchConsultas = async () => {
    try {
      setLoading(true);
      setError(null);
      // Fetch all consultas from the admin endpoint
      ...
      const formattedConsultas = response.data
        .filter((consulta) => {
          ...
          const medicoMatch =
            ...
            (consulta.medico || "")
              .toLowerCase()
              .includes(filters.medico.toLowerCase());

          // 4. Filter by Paciente (loose match)
          const pacienteMatch =
            !filters.paciente ||
          ...
          // 5. Filter by Date Range (if specified in filters)
          let dateMatch = true;
                ...
                <>
                  {consultas.slice(0, displayLimit).map((consulta) => (
                    <ConsultaCard
                      ...
                      {...consulta}
                      onClick={() =>
                        navigate(`/gestor/consultas/${consulta.id}`)
                      }
                    />
                  ))}
